...
import { nTupleFromArray } from "../src/index";
import { uuids, dates, hexadecimals, numbers, isEven, isDate } from './data';

type ParameterDataset<D> = {data: D[], size: number, type?: string};

describe('nTuple Array', () => {
    it('should use default maxItems value', () => {
        const tuples = nTupleFromArray({list: numbers()});
    ...
    it('should use provided maxItems value', () => {
        const tuples = nTupleFromArray({list: numbers(), maxItems: 5});
        for (const tuple of tuples) {
    ...
    it('should work without a match function', () => {
        const tuples = nTupleFromArray({list: uuids(), maxItems: 10});
        for (const tuple of tuples) {
    ...
    it('should use match function when provided', () => {
        ...
        const tuples = nTupleFromArray({list: nums, maxItems: 5, match: isEven});
        for (const tuple of tuples) {
            expect(tuple).toBeArrayOfSize(5);
    ...
    it.each<ParameterDataset<string>>([
        ...
        const tuples = nTupleFromArray({list: data, maxItems: size});
        let allItemsFlattened: (string | undefined | null)[] = [];
        for (const tuple of tuples) {
    ...
    it.each<ParameterDataset<string>>([
        ...
        const tuples = nTupleFromArray({list: data, maxItems: size});
        const allItemsFlattened: (string | undefined | null)[] = [];
        const localTuples: (string | undefined | null)[][] = [];
        for (const tuple of tuples) {
            allItemsFlattened.push(...tuple);
    ...
    it.each<ParameterDataset<number | Date>>([
        ...
        const tuples = nTupleFromArray({list: data, maxItems: size, match: matcher});
